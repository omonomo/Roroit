...
# Custom font generator for Roroit
#
...
lookupIndex_addition="18" # 追加するテーブルのlookupナンバー (lookupの種類を増やした場合変更)
address_init() {
...
name_vendor_url="https://github.com/omonomo/Roroit"
name_designer=""
name_designer_url=""
...
scale_height_block="101" # ボックス要素Y座標拡大率
...
move_y_super="294" # 基本から作成した上付き文字のY座標移動量 (すでにある上付き文字のベースラインのY座標)

center_height_super_sub="197" # 上付き、下付き文字のY座標拡大中心
scale_super_sub2="125" # 上付き、下付き文字の拡大率
move_y_super2="0" # 上付き文字のY座標移動量
move_y_sub="0" # 下付き文字のY座標移動量

move_y_super_base="0" # ベースフォントの上付き文字Y座標移動量 (Latin フォントとベースラインを合わせる)
move_y_sub_base="0" # ベースフォントの下付き文字Y座標移動量 (Latin フォントとベースラインを合わせる)

# latin 括弧移動量 (ベースフォントと中心を合わせる)
move_y_latin_bracket="74"

# latin アンダーバー移動量
move_y_latin_underbar="28"

# 全角アンダーバー移動量 (Latin フォントと高さを合わせる)
move_y_zenkaku_underbar="-30"

# 縦書き全角記号移動量
move_x_vert_colon="63" # ：；
move_x_vert_bar="75" # ｜
move_x_vert_solidus="15" # ／＼
move_x_vert_math="13" # ＝－＜＞
move_y_vert_bbar="15" # ￤
...
calt_init() {
    ...
    move_y_calt_colon=$((move_y_math + 81)) # : のY座標移動量 (= 基準)
    ...
    move_y_calt_bar=$((move_y_math + 28)) # | のY座標移動量 (= 基準)
    ...
    move_y_calt_tilde=$((move_y_math + 57)) # ~ のY座標移動量 (> 基準)
    ...
    move_y_calt_math=$((- move_y_math - 1)) # +-= のY座標移動量 (括弧基準)
    ...
    move_y_calt_colon2="79" # : のY座標移動量 (括弧基準)
    ...
    move_y_calt_bar2="25" # | のY座標移動量 (括弧基準)
...
# Loose 版用
scale_width_latin_loose="93" # Latin フォントの半角英数文字の横拡大率 (Loose 版)
scale_height_latin_loose="93" # Latin フォントの半角英数文字の縦拡大率 (Loose 版)
move_x_hankaku_latin_loose="-22" # Latin フォント全体のX座標移動量 (Loose 版)
scale_width_hankaku_loose="100" # 半角英数文字の横拡大率 (Loose 版)
scale_height_hankaku_loose="100" # 半角英数文字の縦拡大率 (Loose 版)
scale_width_block_loose="98" # 半角罫線素片・ブロック要素の横拡大率 (Loose 版)
...
# Set filenames
origin_latin_regular="RobotoMono-Regular.ttf"
origin_latin_bold="RobotoMono-Bold.ttf"
origin_latin_regular2="NotoSansMono-Regular.ttf"
origin_latin_bold2="NotoSansMono-Bold.ttf"
...
# Q (ss用、突き抜けた尻尾でOと区別しやすく)
    Select(0u0051); RemoveOverlap(); Copy() # Q
    ...
    if (input_list[i] == "${input_latin_regular}")
        ...
        Rotate(23)
        Copy()
        Select(0u0051); PasteWithOffset(50, -170) # Q
    else
        Scale(43, 180)
        Rotate(23)
        Copy()
        Select(0u0051); PasteWithOffset(18, -150) # Q
    endif
...
# Roboto に合わせるため縦横比調整
    ...
    SelectMore(0u039f) # Ο
    SelectMore(0u03f4) # ϴ
    SelectMore(0u1f48, 0u1f4d) # Ὀ - Ὅ
    SelectMore(0u1ff8, 0u1ff9) # ῸΌ
    # キリル文字
    SelectMore(0u041e) # О
    SelectMore(0u0472) # Ѳ
    SelectMore(0u04e6) # Ӧ
    ...
    SelectMore(0u051a) # Ԛ
    # 保管した Q
    SelectMore(${address_store_mod} + 1) # 保管所
    SelectMore(${address_store_mod} + ${num_mod_glyphs} + 1)
    SelectMore(${address_store_mod} + ${num_mod_glyphs} * 2 + 1)
    SelectMore(${address_store_mod} + ${num_mod_glyphs} * 3 + 1)
...
# Open latin font
    ...
    SelectMore(0u005e) # ^
    SelectMore(0u005f) # _
    SelectMore(0u0060) # Grave accent
    SelectMore(0u007b) # {
    SelectMore(0u007c) # |
    SelectMore(0u007d) # }
    SelectMore(0u00a9) # ©
    ...
    SelectMore(0u00d7) # ×
    SelectMore(0u00f7) # ÷
    SelectMore(0u2018, 0u201e) # ‘’‚‛“”„
    SelectMore(0u2032, 0u2033) # ′″
    SelectMore(0u20a0, 0u2125) # ₠ - ℥
    SelectMore(0u221a, 0u22ff) # √ - ⋿
...
# D (ss 用、クロスバーを付加することで少しくどい感じに)
    ...
    Select(0u0044) # D
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-182 + 10, -316)
    else
        PasteWithOffset(-186, -312)
    endif
    SetWidth(${width_latin})
    RemoveOverlap()
    Select(65552);  Clear() # Temporary glyph
...
# Z (ss用、クロスバーを付加してゼェーットな感じに)
    ...
    Select(0u005a) # Z
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-15, -328)
    else
        PasteWithOffset(-13, -323)
    endif
    SetWidth(${width_latin})
    RemoveOverlap()
    Select(65552);  Clear() # Temporary glyph
...
# / ※ 縮小してしまうため一旦消去してコピペ実施
    Select(0u002f); Clear(); DetachAndRemoveGlyphs() # Solidus
    Select(0u005c); Copy() # Reverse Solidus
    Select(0u002f); Paste() # Solidus
...
# ˗ (セリフを付ける)
    ...
    Select(0u02d7) # ˗
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-124, 0)
        PasteWithOffset( 124, 0)
    else
        PasteWithOffset(-114, 0)
        PasteWithOffset( 114, 0)
    endif
    RemoveOverlap()
    SetWidth(${width_latin})
...
# ‐‑ (上げる)
    ...
    SelectMore(0u2011) # ‑ Non-breaking hyphen
    Move(0, 50)
    SetWidth(${width_latin})
...
# Δ (ベースフォントを置き換え) ※ 縮小してしまうため一旦消去してコピペ実施
    Select(0u0394); Clear(); DetachAndRemoveGlyphs() # Δ
    Select(0u2206); Clear(); DetachAndRemoveGlyphs() # ∆
    Select(0u02207); Copy() # ∇
    Select(0u0394); Paste() # Δ
    Rotate(180)
    SetWidth(${width_latin})
    Copy()
    Select(0u2206); Paste() # ∆
...
# Ω (ベースフォントを置き換え) ※ 縮小してしまうため一旦消去してコピペ実施
    Select(0u03a9); Clear(); DetachAndRemoveGlyphs() # Ω
    Select(0u2126); Clear(); DetachAndRemoveGlyphs() # Ω
    Select(0u02127); Copy() # ℧
    Select(0u03a9); Paste() # Ω
    Rotate(180)
    SetWidth(${width_latin})
    Copy()
...
# object replacement character(左にずらす)
    Select(0ufffc) # object replacement character
    Move(-6, 0)
    SetWidth(${width_latin})
...
# ドット0を作成
    ...
    if (input_list[i] == "${input_latin_regular}")
        Scale(90)
        Copy()
        Select(${address_store_zero} + 6)
        PasteWithOffset(0, 15) # 保管所
    else
        Scale(65)
        Copy()
        Select(${address_store_zero} + 6)
        PasteWithOffset(1, 8) # 保管所
    endif
    SetWidth(${width_latin})
    Copy()
...
# / ※ 縮小してしまうため一旦消去してコピペ実施
    Select(0u2215); Clear(); DetachAndRemoveGlyphs() # ∕
...
# ⁄ (Slash と区別するため分割)
    ...
    Select(0u2044) # ⁄
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(213, 393); PasteWithOffset(-213, -393)
    else
        PasteWithOffset(243, 443); PasteWithOffset(-243, -443)
    endif
    OverlapIntersect()
    Rotate(-10)
    SetWidth(${width_latin})
...
# ¦ (隙間を開ける)
    ...
    Select(0u00a6); Paste() # ¦
    Move(0, 627)
    PasteWithOffset(0, -627)

    # |
    Select(0u007c) # |
    Move(0, 536)
    PasteWithOffset(0, -536)
...
# 上付き、下付き文字を追加、sups、subs フィーチャを追加
    ...
    subs = [0u208a, 0u208b, 0u208c, 0u208d, 0u208e] # ₊₋₌₍₎ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(subs[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    subs = [0u2090, 0u2091, 0u2095, 0u1d62,\
            ...
            0u2093] # ₐₑₕᵢ ⱼₖₗₘ ₙₒₚᵣ ₛₜᵤᵥ ₓ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(subs[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    subs = [0u1d66, 0u1d67, 0u1d68, 0u1d69, 0u1d6a, 0u2094] # ᵦᵧᵨᵩᵪₔ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(subs[j]); Paste()
 ...
 #    CorrectDirection()
    ...
    orig = [0u002b, 0u002d, 0u003d, 0u0028, 0u0029] # +-=()
    sups = [0u207a, 0u207b, 0u207c, 0u207d, 0u207e] # ⁺⁻⁼⁽⁾ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    sups = [0u1d43, 0u1d49, 0u02b0, 0u2071,\
            ...
            0u02e3] # ᵃᵉʰⁱ ʲᵏˡᵐ ⁿᵒᵖʳ ˢᵗᵘᵛ ˣ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    sups = [0u1d47, 0u1d9c, 0u1d48, 0u1da0,\
            0u1d4d, 0u02b7, 0u02b8, 0u1dbb] # ᵇᶜᵈᶠ ᵍʷʸᶻ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    sups = [0u1d2c, 0u1d2e, 0u1d30, 0u1d31,\
            ...
            0u1d41, 0u2c7d, 0u1d42] # ᴬᴮᴰᴱ ᴳᴴᴵᴶ ᴷᴸᴹᴺ ᴼᴾᴿᵀ ᵁⱽᵂ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        if (orig[j] == 0u0044) # D
 ...
 #        CorrectDirection()
    ...
    sups = [0u1d5d, 0u1d5e, 0u1d5f, 0u1d60, 0u1d61, 0u1d4a] # ᵝᵞᵟᵠᵡᵊ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    sups = [0u1d44, 0u1d45, 0u1d9b, 0u1d53,\
            ...
            # ᵄᵅᶛᵓ ᶝᵋᶟᶡ ᶢᶣᶤᶥ ᶦᶩᵚᶭ ᶬᶮᶯᶰ ᶱᶲᶳᶴ ᶶᶷᶹᶺ ᶼᶽᶾᶨ ᶫʱʴʵ ʶˀˁˠ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #        CorrectDirection()
    ...
    orig = [0u1d02, 0u025c, 0u0222, 0u1d25, 0u1d1c] # ᴂɜȢᴥᴜ
    sups = [0u1d46, 0u1d4c, 0u1d3d, 0u1d5c, 0u1db8] # ᵆᵌᴽᵜᶸ # グリフ置き換え
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #    CorrectDirection()
    ...
    sups = [0u1d3b, 0u1d4e, 0u02e4] # ᴻᵎˤ # 基本のグリフ無し、上付きのみ
    j = 0
    while (j < SizeOf(sups))
        Select(sups[j])
        Scale(${scale_super_sub2}, ${width_latin} / 2, ${move_y_super} + ${center_height_super_sub})
...
# \ (ss13 用のグリフを作る)
    ...
    Scale(120); Copy()
    if (input_list[i] == "${input_latin_regular}")
        Move(-32, 0); PasteWithOffset(32, 0)
    else
...
# < (ss18 用のグリフを作る)
    ...
    if (input_list[i] == "${input_latin_regular}")
        Scale(175, 92, 351 + ${move_y_math})
    else
        Scale(175, 87, 357 + ${move_y_math})
    endif
    Copy()
    ...
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-72, 0)
    else
        PasteWithOffset(-117, 0)
    endif
    OverlapIntersect()
    ...
    Select(${address_store_ssliga}) # 保管所
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-154 - 20, 0)
    else
        PasteWithOffset(-186 - 5, 0)
    endif
    OverlapIntersect()
...
# > (ss18 用のグリフを作る)
    ...
    Select(65575); Paste() # Temporary glyph
    if (input_list[i] == "${input_latin_regular}")
        Scale(175, 522, 351 + ${move_y_math})
    else
        Scale(175, 527, 357 + ${move_y_math})
    endif
    Copy()
    ...
    Select(${address_store_ssliga} + 1) # 保管所
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(144 + 20, 0)
    else
        PasteWithOffset(175 + 5, 0)
    endif
    OverlapIntersect()
    SetWidth(${width_hankaku})
    Select(65575); Clear() # Temporary glyph
...
# 記号を一部クリア
 ...
 #    Select(0u2500, 0u259f); Clear() # 罫線素片・ブロック要素
    ...
    Select(0u2756); Clear() # ❖
    Select(0u27a1); Clear() # ➡
    Select(0ue000, 0uf6c2); Clear() # 私用領域
    Select(0uf6c4, 0uf8ff); Clear() # 私用領域
...
# ブロック要素を加工 (Powerline対応)
    ...
    Select(0u2591, 0u2593) # 網掛けのみ大きさが異なるための対応
    Scale(100, 112, 0, ${center_height_pl})
    Select(0u2591); Move(0, 17)
    Select(0u2592); Move(0, 17)
    Select(0u2593); Move(0, 76)
...
# 演算子を上下に移動
 ...
 #    math = [0u2243, 0u2248, 0u2252, 0u223c] # ≃≈≒∼
    ...
    while (j < SizeOf(math))
        Select(math[j]);
        Move(0, ${move_y_math} + ${move_y_zenkaku_math} - 1)
        SetWidth(${width_zenkaku})
        j += 1
    endloop
...
# 半角英数記号を全角形にコピー、加工
 ...
 #    SetWidth(${width_zenkaku})
    ...
    Move(75, 0)
    Select(0u0021); Copy() # !
    Select(0u2048); PasteWithOffset(457, 0) # ⁈
    Move(-${move_x_hankaku}, 0)
    SetWidth(${width_zenkaku})
    ...
    Move(76, 0)
    Select(0u003F); Copy() # ?
    Select(0u2049); PasteWithOffset(455, 0) # ⁉
    Move(-${move_x_hankaku}, 0)
    SetWidth(${width_zenkaku})
...
# aalt 1対1
    ...
    orig = [0u0222, 0u1d02, 0u025c, 0u1d25,\
            0u1d1c] # Ȣᴂɜᴥ ᴜ
    sups = [0u1d3d, 0u1d46, 0u1d4c, 0u1d5c,\
            0u1db8] # ᴽᵆᵌᵜ ᶸ
    j = 0
    while (j < SizeOf(orig))
        Select(supb[j])
